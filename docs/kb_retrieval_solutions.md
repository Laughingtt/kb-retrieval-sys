...
| `read_section` | `section_splitter` + `make_snippet` 原语 | 🟡 原语就绪，端点编排待 M3 |
| `list_categories` | 目录结构 + index.md type 分组 | 🟡 待 M3 端点化 |
...
flowchart TD
    ...
    T1 --> IDX[(wiki/index.md按 type 分组 + wikilink)]
    T2 --> IDX
    T3 --> IDX
    T4 --> DOC[(原始 MD 文档PDF/Word/Excel→MD 清洗)]
    T5 -->|不达标| AG
    T5 -->|达标| ANS[整合多文档答案]
