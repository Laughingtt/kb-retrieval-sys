...
| 痛点 | 本方案对策 | 落到哪 |
| --- | --- | --- |
| 精确词搜不到 | BM25 关键词 + bge-m3 向量 **双路 RRF 融合** | §八 检索底座 |
| 又慢又费 | **section 级细粒度切分 + 按需加载**，只返回命中段 | §六 切分、§8.2 查询 |
| 维护不了 | **增量摄入 + 变更检测 + 可重建**，自动只处理变更 | §九 自更新 |
...
**核心优势不是检索算法本身,而是"section 级精准召回 + 按需加载"**：少喂给 LLM、少往返,端到端更快更准。详见 §十二 评估体系验证。
...
flowchart TD
    ...
    WRITE_MD --> INDEX[索引: §7.2 全流程<br/>doc_id→LLM两步→行号回填→index]
...
详见 §五 数据模型 `vectors/` 目录、§5.1 raw不可变原则、§9.6.1 watch 持久化队列。
...
> **分阶段说明**：P0 测试阶段不装向量环境,只用 BM25 单路跑通框架（见 §8.4、§十三）。§十二 的完整对比评估在**向量环境就绪后**运行;P0 验收（§十三）只要求 BM25 路通过,不依赖向量。这样"先看全局框架"与"最终量化对比"分两步,互不阻塞。
...
| # | 风险 | 影响 | 概率 | 缓解措施 |
| --- | --- | --- | --- | --- |
| R1 | **PDF 表格清洗不完整**（复杂表格丢字段） | 数据表/接口文档召回不准 | 中 | PyMuPDF4LLM 权威 + pdfplumber 兜底（F3）；§十二 评估兜底；真实数据复验 |
...
| R4 | **section 切分过粗/过细** | 召回精度受影响 | 中 | 过长 section 二次切分兜底；宽表列分组（F4）；§十二 用例验证调参 |
...
- [ ] 认可"BM25 + 向量双路 RRF 融合"优于纯向量库（§八）
- [ ] 认可"section 级召回 + 按需加载"的省成本思路（§8.2、§8.5）
- [ ] 认可"raw 不可变 + 生成物可重建"的自维护模式（§九）
