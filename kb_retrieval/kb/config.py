...
路径默认基于项目根（kb_retrieval/kb/ 上两级）。LLM 配置默认 DeepSeek，公司内部
OpenAI 兼容端点换 env 即可（CLAUDE.md ③）。全部可被 env 覆盖。

注：本模块只管路径与运行时参数（env）。**领域配置**（wiki 页类型）不经 env，
走 `kb_retrieval/kb/knowledge_base/page_types.yaml`（由 page_type_config.py 加载，
...
_EXTRA_PATHS = {
    ...
    "HASH_PATH": ("kb_retrieval", "kb", "knowledge_base", ".cache", "hash.json"),
    "INGEST_LOG_PATH": ("kb_retrieval", "kb", "knowledge_base", "ingest_log.jsonl"),
}
...
def _resolve_path(name: str) -> Path:
    ...
    if name in _PATH_DIRS:
        default = _PROJECT_ROOT / "kb_retrieval" / "kb" / "knowledge_base" / _PATH_DIRS[name]
        return Path(os.environ.get(name, default))
    if name == "INGEST_CACHE_PATH":
        default = _PROJECT_ROOT / "kb_retrieval" / "kb" / "knowledge_base" / ".cache" / "ingest-cache.json"
