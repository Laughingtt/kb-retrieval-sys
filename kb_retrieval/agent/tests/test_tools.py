# kb_retrieval/agent/tests/test_tools.py
import json
import pytest
from unittest.mock import MagicMock
from kb_retrieval.agent.tools import TOOLS, dispatch, ToolError, extract_grade
...
def test_grep_docs_truncates_snippet_and_caps_hits():
    kb = MagicMock()
    kb.get_search.return_value = {
        "query": "x", "total": 20,
        "hits": [{"doc_id": f"d{i}", "section_id": "s0", "title": "t",
                  "snippet": "字" * 600, "score": 0.1, "source": "bm25"}
                 for i in range(20)],
    }
    out = json.loads(dispatch("grep_docs", {"q": "x", "top_k": 10}, kb))
...
def test_read_section_filters_to_target():
    ...
    out = json.loads(dispatch("read_section", {"slug": "doc1", "section_id": "s1"}, kb))
    assert out == {"slug": "doc1", "section_id": "s1", "title": "B", "body": "bbb"}


def test_read_section_missing_section_raises():
    kb = MagicMock()
    kb.get_document.return_value = {"slug": "doc1", "type": "source", "title": "T",
                                    "updated": None, "sections": [
            {"section_id": "s0", "title": "A", "line_start": 1, "line_end": 3, "body": "aaa"}]}
    with pytest.raises(ToolError):
        dispatch("read_section", {"slug": "doc1", "section_id": "s9"}, kb)


def test_grade_relevance_local_no_l1_call():
    kb = MagicMock()
    args = {"sufficient": True, "missing": [], "next_action": "整合"}
    out = json.loads(dispatch("grade_relevance", args, kb))
    assert out == args
    kb.get_search.assert_not_called()  # 确认没调 KB Service
