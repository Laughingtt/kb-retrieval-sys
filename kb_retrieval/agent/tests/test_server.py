# kb_retrieval/agent/tests/test_server.py
import json
import pytest
...
def _client_with_agent(agent_result):
    ...
    """
    from kb_retrieval.agent.server import app
    fake = MagicMock()
    fake.run.return_value = agent_result
    patcher = patch("kb_retrieval.agent.server._build_agent", return_value=fake)
...
def test_health(monkeypatch):
    monkeypatch.setattr("kb_retrieval.agent.config.LLM_API_KEY", "sk-test")
    from kb_retrieval.agent.server import app
    with patch("kb_retrieval.agent.server._l1_reachable", return_value=True):
        c = TestClient(app)
        r = c.get("/health")
    assert r.status_code == 200
    body = r.json()
