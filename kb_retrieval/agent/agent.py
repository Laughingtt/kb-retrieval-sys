...
from kb_retrieval.agent.prompts import SYSTEM_PROMPT, FINALIZE_HINT, GAP_HINT
from kb_retrieval.agent.tools import TOOLS, dispatch, extract_grade, ToolError

__all__ = ["AgentLoop"]


class AgentLoop:
    def __init__(self, llm: OpenAI | Any = None, kb: KBClient | None = None) -> None:
        self._llm = llm or OpenAI(base_url=config.LLM_BASE_URL,
                                  api_key=config.LLM_API_KEY, timeout=config.LLM_TIMEOUT)
        self._l1 = kb or KBClient()
