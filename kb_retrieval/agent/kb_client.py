"""薄 httpx 客户端 —— 封装 KB Service 的 6 个 GET 端点。只读（硬约束 2）。"""
...
class KBClient:
    def __init__(self, base_url: str | None = None, timeout: float | None = None) -> None:
        self._client = httpx.Client(
            base_url=base_url or config.KB_BASE_URL,
            timeout=timeout or config.KB_TIMEOUT,
        )
    ...
    def _get(self, path: str, **params: Any) -> Any:
        ...
        if r.status_code >= 400:
            try:
                ...
                detail = r.text
            raise KBClientError(r.status_code, str(detail))
        return r.json()
