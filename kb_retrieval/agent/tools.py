# kb_retrieval/agent/tools.py
"""5 工具：OpenAI function-calling schema + dispatch 执行函数。

前 4 个薄封装 KBClient（只读 GET）；grade_relevance 本地无 KB Service 调用。
...
def _t_list_categories(kb: KBClient, args: dict) -> Any:
    return kb.get_categories()


def _t_list_documents(kb: KBClient, args: dict) -> Any:
    return kb.get_documents(type=args.get("type"), page=args.get("page", 1),
                            page_size=args.get("page_size", 50))


def _t_grep_docs(kb: KBClient, args: dict) -> Any:
    res = kb.get_search(_require(args, "q"), top_k=args.get("top_k", 10))
    hits = []
...
def _t_read_section(kb: KBClient, args: dict) -> Any:
    slug = _require(args, "slug")
    sid = _require(args, "section_id")
    doc = kb.get_document(slug)
...
def _t_grade_relevance(kb: KBClient, args: dict) -> Any:
    # 本地：不调 KB Service。原样返回 LLM 产出的结构化判定。
...
def dispatch(name: str, args: dict, kb: KBClient) -> str:
    ...
    try:
        ...
        # KB Service 错误塞回给 LLM 作为 tool result（Agent 自主决定重试/告知用户）
        return json.dumps({"error": f"KB {e.status}: {e.detail}"}, ensure_ascii=False)
